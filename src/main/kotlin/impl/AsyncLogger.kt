...
class AsyncLogger(private var config: LoggerConfig) : Logger {
    ...
    override fun debug(logMessage: String?) {
        logMessageQueue.addLast(logMessage?.let {
            LogMessage(LocalDateTime.now().format(config.formatter), LogLevel.DEBUG,
                it
            )
        })
    }
    ...
    override fun info(logMessage: String?) {
        logMessageQueue.addLast(logMessage?.let {
            LogMessage(LocalDateTime.now().format(config.formatter), LogLevel.INFO,
                it
            )
        })
    }
    ...
    override fun warn(logMessage: String?) {
        logMessageQueue.addLast(logMessage?.let {
            LogMessage(LocalDateTime.now().format(config.formatter), LogLevel.WARN,
                it
            )
        })
    }
    ...
    override fun error(logMessage: String?) {
        logMessageQueue.addLast(logMessage?.let {
            LogMessage(LocalDateTime.now().format(config.formatter), LogLevel.ERROR,
                it
            )
        })
    }
    ...
    override fun fatal(logMessage: String?) {
        logMessageQueue.addLast(logMessage?.let {
            LogMessage(LocalDateTime.now().format(config.formatter), LogLevel.FATAL,
                it
            )
        })
    }
